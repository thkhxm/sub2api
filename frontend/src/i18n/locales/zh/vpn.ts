export default {
  vpn: {
    ...
    usageProgress: '已用 {used}，每月额度 {quota}，占比 {percent}',
    sort: '排序', sortNewest: '最新创建优先', sortUsageDesc: '已用流量：从高到低', sortUsageAsc: '已用流量：从低到高',
    ...
    clash: 'Clash / Mihomo', base64: '通用 Base64 / v2ray', singbox: 'sing-box', copy: '复制地址',
    egressSubscription: '运维订阅', egressHint: '用于管理员运维连接，与个人订阅独立。',
    unlimitedTraffic: '不限流量', expiry: '有效期', noExpiry: '无到期',
    egressPending: '运维订阅暂未就绪，请刷新状态。生效并允许连接后可复制地址。',
    egressFailed: '运维订阅未能生效，请刷新重试；持续失败时请检查节点连接。',
    egressLoadFailed: '无法获取运维订阅，请刷新重试或检查节点连接。',
    ...
    createHint: '自动选择健康且启用、可分配额度充足且分配占比最低的节点；管理员代开不受余额限制。已有订阅将返回原订阅。',
    quotaGiB: '每月额度（GiB）', invalidQuota: '请输入大于 0 的有效额度，字节值不能超过安全整数上限。',
    invalidHttps: '节点地址必须使用 HTTPS，且不能包含用户名或密码。',
    retry: '重试原操作', revoke: '选择节点并轮换', revokeConfirm: '将在当前节点“{name}”原地轮换，不新增占额，并保留本期已用流量。原订阅地址和连接凭据失效，旧连接断开；此用户所有设备都需要重新导入订阅。是否继续？',
    revokeMigrateConfirm: '将订阅迁移到“{name}”，并保留本期已用流量。原订阅地址和连接凭据失效，旧连接断开；此用户所有设备都需要重新导入订阅。是否继续？',
    selectRotationServer: '选择目标节点', currentServer: '当前节点',
    selectRotationServerHint: '请手动选择目标节点。选当前节点为原地轮换，选其他节点为迁移。分配占比按有限额账号的月额度总和计算，含待生效预占；不扣已用流量，不计不限量运维账号。迁移按完整月额预占，最终容量以提交时校验为准。',
    allocatedQuota: '已分配额度', allocatableQuota: '可分配额度', insufficientAllocation: '可分配额度不足', unknownAllocation: '容量尚未确认，请刷新状态',
    noRotationServer: '暂无可选节点，请检查节点状态与可分配额度。',
