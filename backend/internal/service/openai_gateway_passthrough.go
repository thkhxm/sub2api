...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	for {
		...
		detachedBaseCtx := context.Background()
		if ctx != nil {
			detachedBaseCtx = context.WithoutCancel(ctx)
		}
		upstreamCtx, cancelUpstreamCtx := context.WithCancel(detachedBaseCtx)
		stopHeaderWaitCancel := func() bool { return false }
		if ctx != nil {
			// 响应头到达前还没有可收集的 usage；客户端断开时立即取消等待，
			// 避免 detached context 继续占用账号与 HTTP 并发槽。
			stopHeaderWaitCancel = context.AfterFunc(ctx, cancelUpstreamCtx)
		}
		upstreamReq, buildErr := s.buildUpstreamRequestOpenAIPassthrough(upstreamCtx, c, account, body, token)
		if buildErr != nil {
			stopHeaderWaitCancel()
			cancelUpstreamCtx()
			return nil, buildErr
		}
		...
		resp.Body = &openAICancelOnCloseReadCloser{ReadCloser: resp.Body, cancel: cancelUpstreamCtx}
...
// openAICancelOnCloseReadCloser 在消费响应体期间保持脱离客户端取消的上游
// context，并在关闭响应体时同步释放该 context。
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	processSSELine := func(line string) (*openaiStreamingResultPassthrough, error) {
	...
	finalizeStream := func() (*openaiStreamingResultPassthrough, error) {
		...
		if clientDisconnected {
			logOpenAISuccessMissingUsage(ctx, c, account, resp, usage, terminalEventType, true)
			return resultWithUsage(), nil
		}
		var ctxErr error
		logCtx := ctx
	...
	for {
		...
		case ev, ok := <-events:
			...
			if result, err := processSSELine(ev.line); err != nil {
				return result, err
			}
