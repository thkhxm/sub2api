...
func scanVPNServer(row vpnScanner) (*service.VPNServer, error) {
	...
	if len(allocation) > 0 {
		if err = json.Unmarshal(allocation, &v.AllocationSnapshot); err != nil {
			return nil, err
		}
	}
	return &v, nil
...
func (r *vpnRepository) ListServers(ctx context.Context) ([]service.VPNServer, error) {
	...
	return list, hydrateVPNAllocations(ctx, r.db, list, "")
}
func (r *vpnRepository) GetServer(ctx context.Context, id int64) (*service.VPNServer, error) {
	v, err := scanVPNServer(r.db.QueryRowContext(ctx, `SELECT `+vpnServerColumns+` FROM vpn_servers v WHERE v.id=$1`, id))
	if err != nil {
		return nil, err
	}
	list := []service.VPNServer{*v}
	if err := hydrateVPNAllocations(ctx, r.db, list, ""); err != nil {
		return nil, err
	}
	return &list[0], nil
}
func (r *vpnRepository) SaveServer(ctx context.Context, v *service.VPNServer) (*service.VPNServer, error) {
	if v.ID == 0 {
		...
		result, err := r.db.ExecContext(ctx, `UPDATE vpn_servers SET name=$2,base_url=$3,admin_username=$4,credentials_encrypted=$5,enabled=$6,traffic_quota_bytes=$7,traffic_used_offset_bytes=$8,traffic_offset_period_start=$9,healthy=false,last_checked_at=NULL,updated_at=now() WHERE id=$1 AND (base_url=$3 OR (NOT EXISTS(SELECT 1 FROM vpn_subscriptions WHERE server_id=$1) AND NOT EXISTS(SELECT 1 FROM vpn_subscription_binding_history WHERE server_id=$1) AND NOT EXISTS(SELECT 1 FROM vpn_operations WHERE action='migrate' AND status IN ('pending','running','failed') AND (payload->'migration'->>'target_server_id')::bigint=$1)))`, v.ID, v.Name, v.BaseURL, v.AdminUsername, v.CredentialsEncrypted, v.Enabled, v.TrafficQuotaBytes, v.TrafficUsedOffsetBytes, v.TrafficOffsetPeriodStart)
		if err != nil {
			return nil, err
		}
...
func (r *vpnRepository) UpdateServerHealth(ctx context.Context, id int64, m *service.VPNServerMeta, message string) error {
	...
	var snapshot *service.VPNNodeAllocation
	if m.AllocatedQuotaBytes != nil {
		snapshot = &service.VPNNodeAllocation{AllocatedQuotaBytes: *m.AllocatedQuotaBytes, ManagedQuotaBytes: m.ManagedQuotaBytes}
	}
	allocation, _ := json.Marshal(snapshot)
	_, err := r.db.ExecContext(ctx, `UPDATE vpn_servers SET healthy=$2,health_error=$3,personal_user_count=$4,known_owner_refs=$5,traffic_snapshot=$6,allocation_snapshot=$7,last_checked_at=now() WHERE id=$1`, id, m.Healthy, message, m.PersonalUserCount, string(refs), string(traffic), string(allocation))
	return err
}
...
func (r *vpnRepository) Eligibility(ctx context.Context, id int64) (bool, string, error) {
	...
	err := r.db.QueryRowContext(ctx, `SELECT status='active' AND deleted_at IS NULL,balance>0 FROM users WHERE id=$1`, id).Scan(&active, &balance)
	if err != nil {
	...
	group, err := r.UserVPNGroup(ctx, id)
	if err != nil {
		return false, "", err
	}
	...
	for _, server := range servers {
		if server.Enabled && server.Healthy && server.LastCheckedAt != nil && time.Since(*server.LastCheckedAt) <= 2*time.Minute && server.AllocationAvailableBytes != nil && *server.AllocationAvailableBytes >= group.QuotaBytes {
			return true, "", nil
		}
	}
...
func (r *vpnRepository) Reserve(ctx context.Context, userID int64, admin bool, actor int64, owner string) (*service.VPNSubscription, error) {
	...
	var id int64
	var deleting, serverEnabled, migrating bool
	err = tx.QueryRowContext(ctx, `SELECT s.id,s.delete_requested_at IS NOT NULL,v.enabled,EXISTS(SELECT 1 FROM vpn_operations o WHERE o.subscription_id=s.id AND o.action='migrate' AND o.status IN ('pending','running','failed')) FROM vpn_subscriptions s JOIN vpn_servers v ON v.id=s.server_id WHERE s.user_id=$1 AND s.deleted_at IS NULL`, userID).Scan(&id, &deleting, &serverEnabled, &migrating)
	if err == nil {
		if deleting {
			return nil, service.ErrVPNBusy
		}
		if !serverEnabled && !migrating {
			return nil, service.ErrVPNServerDisabled
		}
	...
	serverID, err := selectVPNServerByQuota(ctx, tx, vpnNodeRequest{RequiredQuota: quota, OwnerRef: owner})
	if err != nil {
		return nil, err
	}
...
func (r *vpnRepository) Queue(ctx context.Context, id, actor int64, p service.VPNOperationPayload) error {
	...
	var migrating bool
	if err = tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM vpn_operations WHERE subscription_id=$1 AND action='migrate' AND status IN ('pending','running','failed'))`, id).Scan(&migrating); err != nil {
		return err
	}
	if migrating {
		return service.ErrVPNBusy
	...
	if p.TargetServerID > 0 {
		sub, err := scanVPNSub(tx.QueryRowContext(ctx, `SELECT `+vpnSubColumns+vpnSubJoin+` WHERE s.id=$1`, id))
		if err != nil {
			return err
		}
		if p.TargetServerID == sub.ServerID {
			if err := validateVPNRotationNode(ctx, tx, sub.ServerID); err != nil {
				return err
			}
		} else {
			newOwner := uuid.NewString()
			target, err := selectVPNServerByQuota(ctx, tx, vpnNodeRequest{SourceID: sub.ServerID, RequestedID: p.TargetServerID, RequiredQuota: sub.QuotaBytes, OwnerRef: newOwner})
			if err != nil {
				return err
			}
			p.Action = "migrate"
			p.Migration = newVPNMigration(sub, target, newOwner, true)
...
func (r *vpnRepository) Claim(ctx context.Context) (*service.VPNOperation, error) {
	...
	err := r.db.QueryRowContext(ctx, `UPDATE vpn_operations SET status='running',lease_until=now()+interval '90 seconds',lease_token=$1,attempts=attempts+1 WHERE id=(SELECT id FROM vpn_operations WHERE status IN ('pending','running') AND available_at<=now() AND (lease_until IS NULL OR lease_until<now()) ORDER BY available_at,created_at FOR UPDATE SKIP LOCKED LIMIT 1) RETURNING id,subscription_id,payload,attempts,dispatched`, o.LeaseToken).Scan(&o.ID, &o.SubscriptionID, &o.Payload, &o.Attempts, &o.Dispatched)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, nil
	}
	return &o, err
...
func (r *vpnRepository) List(ctx context.Context, f service.VPNFilter) ([]service.VPNSubscription, int, error) {
	order := "s.created_at"
	if f.SortBy == "used_bytes" {
		order = "(s.snapshot->>'used_traffic')::bigint"
	} else if f.SortBy != "" && f.SortBy != "created_at" {
		return nil, 0, service.ErrVPNInvalid
	}
	direction := "DESC"
	if f.SortOrder == "asc" {
		direction = "ASC"
	} else if f.SortOrder != "" && f.SortOrder != "desc" {
		return nil, 0, service.ErrVPNInvalid
	}
	...
	rows, err := r.db.QueryContext(ctx, `SELECT `+vpnSubColumns+vpnSubJoin+where+` ORDER BY `+order+` `+direction+` NULLS LAST,s.id DESC LIMIT $4 OFFSET $5`, f.Query, f.ServerID, f.Status, f.PageSize, (f.Page-1)*f.PageSize)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
...
func (r *vpnRepository) UserOwnerRefs(ctx context.Context, userID int64) (map[int64][]string, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT server_id,owner_ref FROM vpn_subscriptions WHERE user_id=$1 UNION SELECT server_id,owner_ref FROM vpn_subscription_binding_history WHERE user_id=$1 ORDER BY server_id,owner_ref`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
