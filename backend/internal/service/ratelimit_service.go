...
const upstreamImageBridgeModelReason = "upstream_image_bridge_model_unavailable"
...
func (s *RateLimitService) HandleUpstreamModelNotFound(ctx context.Context, account *Account, requestedModel string, statusCode int, responseBody []byte) bool {
	...
	if reason == upstreamCodexPlanGatedModelReason && isOpenAIOAuthAccount(account) {
		if mainModel := openAIImagesBridgeModelFromContext(ctx); mainModel != "" && upstreamImageBridgeModelRejected(responseBody, mainModel) {
			// 上游拒绝的是调用工具的主模型，不能把这次拒绝写成 gpt-image-* 配额冷却。
			modelKey = mainModel
			reason = upstreamImageBridgeModelReason
		}
	}
	if modelKey == "" {
		return false
