---
name: "imagegen"
description: "Generate or edit images (photos, illustrations, mockups, sprites, banners, hero shots, product shots, concept art, transparent cutouts, avatars, posters, wallpapers, covers). **This is the DEFAULT skill for any image-creation or image-editing request — invoke it automatically, do not fall back to SVG/HTML/CSS unless the user explicitly asks for vector/code output.** TRIGGER on English phrasings like: generate/create/make/draw/produce an image / picture / photo / illustration / banner / poster / hero image / cover / wallpaper / avatar / icon image / logo image / concept art / sprite / mockup / texture / cutout, or 'edit this image / change the background / make it transparent / remove the background'. ALSO TRIGGER on Chinese phrasings (high priority — users often phrase image requests in Chinese): 生成图片 / 画图 / 画一张 / 画个 / 画一只 / 做一张图 / 做张图 / 来一张 / 出一张 / 帮我画 / 帮我生成 / 给我画 / 给我生成 / 画只小猫 / 画只小狗 / 插画 / 海报 / 壁纸 / 头像 / 封面 / 概念图 / 渲染图 / 产品图 / 宣传图 / 抠图 / 透明背景 / 去背景 / 改背景 / 修图 / 编辑这张图. Output defaults to bitmap (PNG/JPEG/WebP). SKIP only when: (a) the user explicitly says SVG / 矢量 / vector / icon-set / HTML / CSS / canvas / code-native, (b) the request is to extend an existing in-repo SVG/icon/logo system, or (c) it's a tiny diagram/wireframe naturally written as code."
---
...
## ⚠️ RUNTIME ENVIRONMENT — HIGHEST PRIORITY (PunkcodeAI / opencode), OVERRIDES ALL "built-in tool mode" TEXT BELOW

This deployment runs inside **opencode (PunkcodeAI desktop)**. There is **NO** built-in `image_gen` / `image_generation` / `view_image` tool here. Therefore:

- **NEVER call the `image_generation`, `image_gen`, or `view_image` tools.** They do **not** exist in this host; calling them fails immediately with `unavailable tool` and produces no image. If you see an `image_generation` tool in the tool list, **ignore it — do not call it.**
- **ALWAYS use CLI mode.** To generate an image, call the **bash/shell tool** to run the bundled Python script (its absolute path is shown in this skill's file listing):
  ```
...
- **Built-in tool mode:** built-in `image_gen` tool (only present in some hosts, e.g. Codex). Use it when available; it does not require `OPENAI_API_KEY`.
- **CLI mode (default in opencode and any host without the built-in tool):** `scripts/image_gen.py`. Credentials are auto-discovered: `OPENAI_API_KEY` env var → opencode config (`opencode.jsonc`/`opencode.json` at `provider.openai.options.apiKey`, with `baseURL` honored as `OPENAI_BASE_URL`) → opencode `auth.json`. **Do NOT pre-flight `$env:OPENAI_API_KEY` and ask the user to set it** — just invoke the script; it reports missing credentials only if all three sources fail.

**Mode selection (no confirmation prompts for environment-driven choices):**
- If the built-in `image_gen` tool exists in this host, use it by default.
- If the built-in tool does NOT exist (e.g. you are running inside opencode), **silently use the CLI as the default path — do NOT ask the user to confirm**. Just generate the image.
- The only times you must ask for confirmation are: (a) downgrading from `gpt-image-2` to `gpt-image-1.5` for true-transparency, and (b) the user has already opted into CLI mode but is requesting something that needs a destructive override.
...
- If the built-in tool is unavailable in this host (typical for opencode), **silently use `scripts/image_gen.py` as the default** with the auto-discovered opencode OpenAI credentials. Do NOT ask the user whether to continue — just generate the image. Only surface a prompt if all three credential sources are empty, or if the request specifically needs a `gpt-image-1.5` transparency downgrade.
...
## Workflow
1. Decide the top-level mode: if the built-in `image_gen` tool exists in this host, use it; otherwise (e.g. opencode) **silently use `scripts/image_gen.py`** without asking the user. Only `gpt-image-1.5` true-transparency downgrades still require explicit confirmation.
...
```text
This likely needs true native transparency. The default built-in path uses a chroma-key background plus local removal, but true transparency requires the CLI fallback with gpt-image-1.5 because gpt-image-2 does not support background=transparent. The CLI will reuse opencode's configured OpenAI credentials automatically. Should I proceed with that CLI fallback?
```
...
### Environment
- The CLI fallback auto-discovers OpenAI credentials in this order:
  1. `OPENAI_API_KEY` env var (and optional `OPENAI_BASE_URL`)
  2. opencode config file: `~/.config/opencode/opencode.jsonc` (or `opencode.json`, also `$XDG_CONFIG_HOME/opencode/...`, Windows `%APPDATA%\opencode\...`) at `provider.openai.options.apiKey`; if `baseURL` is present it is honored as `OPENAI_BASE_URL`.
  3. opencode `auth.json` (under `~/.local/share/opencode/`, `$XDG_DATA_HOME/opencode/`, macOS `~/Library/Application Support/opencode/`, or Windows `%APPDATA%\opencode\`) at the `openai` provider entry.
- **Do not pre-flight check `$env:OPENAI_API_KEY` and refuse to run.** Just invoke `scripts/image_gen.py` — when credentials are missing from all three sources it prints an actionable error.
- Do not ask the user for `OPENAI_API_KEY` when using the built-in `image_gen` tool.
- Never ask the user to paste the full key in chat. If all three sources are empty, ask them to set it locally (env var or `opencode.jsonc`) and confirm when ready.

If all three credential sources are empty, give the user these options:
1. Add an `openai` provider to `~/.config/opencode/opencode.jsonc` (`provider.openai.options.apiKey`, optionally `baseURL`).
2. Or run `opencode auth login openai` so it lands in `auth.json`.
3. Or export `OPENAI_API_KEY` (and optional `OPENAI_BASE_URL`) in their shell.
Create an OpenAI key here if needed: https://platform.openai.com/api-keys
...
- `scripts/image_gen.py`: CLI implementation. Use it as the default path when the built-in `image_gen` tool is not present in the host (e.g. opencode). When the built-in tool IS present, only invoke this script if the user explicitly chooses CLI mode or confirms a `gpt-image-1.5` transparency downgrade.
