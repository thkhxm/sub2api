...
def _opencode_config_dirs() -> List[Path]:
    dirs: List[Path] = []
...
def _opencode_data_dirs() -> List[Path]:
    dirs: List[Path] = []
    xdg_data = os.getenv("XDG_DATA_HOME")
    if xdg_data:
        dirs.append(Path(xdg_data) / "opencode")
    home = Path.home()
...
def _load_opencode_provider_config() -> Tuple[Optional[str], Optional[str], Optional[Path]]:
    for cfg_dir in _opencode_config_dirs():
        for name in ("opencode.jsonc", "opencode.json"):
            ...
            openai_cfg = provider.get("openai") if isinstance(provider, dict) else None
            options = openai_cfg.get("options") if isinstance(openai_cfg, dict) else None
...
def _load_opencode_auth_key() -> Tuple[Optional[str], Optional[Path]]:
    for cfg_dir in _opencode_data_dirs():
        path = cfg_dir / "auth.json"
        if not path.exists():
            continue
        try:
...
def _ensure_api_key(dry_run: bool) -> None:
    ...
    api_key, base_url, cfg_path = _load_opencode_provider_config()
    if api_key:
        os.environ["OPENAI_API_KEY"] = api_key
        if base_url and not os.getenv("OPENAI_BASE_URL"):
            os.environ["OPENAI_BASE_URL"] = base_url
        print(
            f"OPENAI_API_KEY loaded from opencode config ({cfg_path}).",
            file=sys.stderr,
        )
        if base_url:
            print(f"OPENAI_BASE_URL set to {os.environ['OPENAI_BASE_URL']}.", file=sys.stderr)
        return

    auth_key, auth_path = _load_opencode_auth_key()
    if auth_key:
        os.environ["OPENAI_API_KEY"] = auth_key
        print(
            f"OPENAI_API_KEY loaded from opencode auth.json ({auth_path}).",
            file=sys.stderr,
        )
        return
    ...
    if dry_run:
        ...
        return
    _die(
        "OPENAI_API_KEY is not set, and no usable `openai` entry was found in opencode "
        "config (opencode.jsonc/opencode.json) or auth.json. Export OPENAI_API_KEY, set "
        "provider.openai.options.apiKey in opencode.jsonc, or run `opencode auth login openai`."
    )
